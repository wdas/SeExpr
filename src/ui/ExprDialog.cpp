...
void ExprDialog::setupHelp(QTabWidget* tab)
{
    ...
    QFile *helpDoc = new QFile(QCoreApplication::applicationDirPath() +
...
void ExprDialog::applyExpression()
{
    ...
    if(grapher->expr.varmap.size()>0){
        ...
        int count=0;
        for(BasicExpression::VARMAP::iterator i=grapher->expr.varmap.begin();
    ...
    if(grapher->expr.funcmap.size()>0){
        ...
        int count=0;
        for(BasicExpression::FUNCMAP::iterator i=grapher->expr.funcmap.begin();
