...
ExprControlCollection::~ExprControlCollection()
{
    ...
    ExprAddDialog::ExprAddDialog(int& count,QWidget* parent)
        :QDialog(parent)
    {
...
rebuildControls(const QString& expressionText,std::vector<QString>& variables)
{
    ...
    if(editableExpression && editableExpression->controlsMatch(*newEditable)){
        ...
        for(size_t i=0;i<editableExpression->size();i++){
            ...
            if(NumberEditable* x=dynamic_cast<NumberEditable*>(editable)) widget=new NumberControl(i,x);
            else if(VectorEditable* x=dynamic_cast<VectorEditable*>(editable)) widget=new VectorControl(i,x);
            else if(StringEditable* x=dynamic_cast<StringEditable*>(editable)) widget=new StringControl(i,x);
            else if(CurveEditable* x=dynamic_cast<CurveEditable*>(editable)) widget=new CurveControl(i,x);
            else if(ColorCurveEditable* x=dynamic_cast<ColorCurveEditable*>(editable)) widget=new CCurveControl(i,x);
