...
inline void printVal(std::stringstream& stream,const SeExpr2::Vec3d& v){stream<<"["<<v[0]<<","<<v[1]<<","<<v[2]<<"]";}
...
struct AnimCurveEditable:public Editable
{
    ...
    AnimCurveEditable(const std::string& name,int startPos,int endPos)
        :Editable(name,startPos,endPos),curve(animlib::AnimAttrID())
    {}
...
struct ColorSwatchEditable:public Editable
{
    ...
    ColorSwatchEditable(const std::string& name,
