...
ExprColorFrame::ExprColorFrame(SeExpr2::Vec3d value, QWidget* parent) :
...
ExprColorWidget::ExprColorWidget(SeExpr2::Vec3d value, int index,
...
void ExprColorSwatchWidget::addSwatch(SeExpr2::Vec3d &val, int index)
{
    ...
    ExprColorWidget *widget = new ExprColorWidget(val, index,
                                                          _indexLabel, this);
    ...
    connect(swatchFrame,SIGNAL(deleteSwatch(ExprColorFrame *)),
            this,SLOT(removeSwatch(ExprColorFrame *)));
    emit swatchAdded(index, val);
}
