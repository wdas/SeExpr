...
CEGraphSeg::CEGraphSeg(CEGraphCurve* curve, int index)
    ...
    //    std::cerr<<"we have "<<_keytime<<" "<<_keyval<<" out "<<_outtime<<" "<<_outval<<" tang
...
bool CEGraphSeg::beginPart(Part part) {
    ...
    bool active = (part == activePart && _index == activeSeg && _curve->index() == activeCurve);
    bool selected = _selected || (part == Key && _pendingSel);
    ui->setColor(selected ? (active ? CEGraphUI::HiSelectedColor : CEGraphUI::SelectedColor)
                          : (active ? CEGraphUI::HiCurveColor : CEGraphUI::CurveColor));
    return 1;
}
