...
} TangTypes[] = {{"fixed", animlib::AnimKeyframe::kTangentFixed},
...
} InfinityTypes[] = {{"constant", animlib::AnimCurve::kInfinityConstant},
...
void CESegEditUI::handleChanged() {
    ...
    if (!ok) {
        std::cerr << "frame is invalid" << std::endl;
        return;
    }
