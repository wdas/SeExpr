...
void CEGraphUI::pickActive(int x, int y) {
    ...
    while (numHits--) {
        ...
        if (numNames >= 3) {
            ...
            if (part > activePart || (part == activePart && (selected || !activeSelected))) {
                activeCurve = curve;
                activeSeg = seg;
                activePart = part;
    ...
    if (activeCurve != _activeCurve || activeSeg != _activeSeg || activePart != _activePart) {
...
void CEGraphUI::paintSelBox() {
    ...
    glEnable(GL_SCISSOR_TEST);
    glScissor(_scalesWidth, _scalesHeight, width() - _scalesWidth, height() - _scalesHeight);
...
void CEGraphUI::printStringAt(double x, double y, const char* str) {
    ...
    for (const char* cp = str; *cp; cp++) {
        if (*cp < ' ' || ((unsigned char)*cp) >= 128) {
            ...
            std::cerr << "Illegal char in GL string - skipping: " << str << std::endl;
            return;
        }
    }
    ...
    if (e->button() == Qt::LeftButton) {
        if (_activeCurve >= 0 && _activeSeg >= 0) {
            if (auto key = getKey(_activeCurve, _activeSeg)) {
                ...
                bool selected = 0;
                _tool->isSegmentSelected(selected, _activeCurve, _activeSeg);
                int keystate =
