    ...
    if (!_selListValid) {
        ...
        for (int i = 0; i < numCurves(); i++) {
            ...
            if (_curves[i]->selected) {
                ...
                int segindex = 0;
                for (seg = curve->segsSelected.begin(); seg != curve->segsSelected.end(); seg++) {
...
err::Result CETool::getSelectedCurve(int& curve) {
    ...
    ERETIF(index < 0 || index >= (int)_curves.size(), errINTERNAL, "Bad curve index");
    curve = index;
    return errSUCCESS;
}
...
void keysUpdated(animlib::AnimCurve* curve, int keyStart, int keyEnd, bool regenerateWeights) {
    ...
    for (int i = keyStart; i < keyEnd; i++) {
        ...
        animlib::AnimKeyframe::tangentType inTangentType = curr->getInTangentType(),
...
void CETool::setSelectedSegment(double frame,
                                double val,
                                double inAngle,
                                double outAngle,
                                double inWeight,
...
void CETool::setSegment(int curve,
                        int seg,
                        double frame,
                        double val,
                        double inAngle,
...
void CETool::deleteSegments() {
    ...
    for (int i = 0; i < numCurves(); i++) {
        ...
        for (seg = curve->segsSelected.rbegin(); seg != curve->segsSelected.rend(); seg++) {
