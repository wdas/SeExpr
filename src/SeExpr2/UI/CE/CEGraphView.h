...
class CEGraphView {
  public:
    CEGraphView() : _vx(0), _vy(0), _vw(1), _vh(1), _px(0), _py(0), _pw(1), _ph(1)
    {
    }
    void setView(double vx, double vy, double vw, double vh, int px, int py, int pw, int ph)
    {
        _vx = vx;
        _vy = vy;
        _vw = vw;
    ...
    double pxd(double vx) const
    {
        return vx * _vxScale + _vxTrans;
    }
    double pyd(double vy) const
    {
        return vy * _vyScale + _vyTrans;
    }
    int px(double vx) const
    {
        return int(floor(vx * _vxScale + _vxTrans));
    }
    int py(double vy) const
    {
        return int(floor(vy * _vyScale + _vyTrans));
    }

    // pix coord to view coord conversion
    double vx(int px) const
    {
        return (px - _vxTrans) / _vxScale;
    }
    double vy(int py) const
    {
        return (py - _vyTrans) / _vyScale;
    }
