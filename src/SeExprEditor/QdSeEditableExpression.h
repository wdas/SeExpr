...
class QdSeEditableExpression{
    ...
    typedef std::vector<QdSeEditable*> Editables;
    std::vector<QdSeEditable*> _editables; // control that can edit the expression
    std::vector<std::string> _variables;
public:
    QdSeEditableExpression();
    ~QdSeEditableExpression();
    ...
    bool controlsMatch(const QdSeEditableExpression& other) const;

    /// Update the string refered to into the controls (this is only valid if controlsmatch)
    void updateString(const QdSeEditableExpression& other);

    /// Access an editable parameter
    QdSeEditable* operator[](const int i){return _editables[i];}
