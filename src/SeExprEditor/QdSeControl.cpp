...
void QdSeSlider::mousePressEvent(QMouseEvent* e)
{
    mouseMoveEvent(e);
}

void QdSeSlider::mouseMoveEvent(QMouseEvent* e)
{
...
void QdSeSlider::paintEvent(QPaintEvent* e)
{
    QPainter p(this);
...
QdSeChannelSlider::QdSeChannelSlider(int id, QWidget* parent)
 : QWidget(parent), _id(id), _value(0)
{
}

void QdSeChannelSlider::paintEvent(QPaintEvent* e)
{
...
void QdSeChannelSlider::mousePressEvent(QMouseEvent* e)
{
    mouseMoveEvent(e);
}


void QdSeChannelSlider::mouseMoveEvent(QMouseEvent* e)   
{
    setValue(clamp(float(e->x()-1) / (width()-3), 0, 1));
}


void QdSeChannelSlider::setValue(float value)
{
...
QdSeControl::QdSeControl(int id,QdSeEditable* editable,bool showColorLink)
...
void QdSeControl::linkStateChange(int state)
{
    if(_updating) return;
...
void QdSeControl::linkDisconnect(int newId)
{
    if(newId!=_id){
        _updating = 1;
...
QdSeNumberControl::QdSeNumberControl(int id,QdSeNumberEditable* editable)
    :QdSeControl(id,editable,false),_numberEditable(editable)
{
    ...
    _slider = new QdSeSlider(Qt::Horizontal,this);
...
void QdSeNumberControl::sliderChanged(int value)
{
    if (_updating) return;
...
void QdSeNumberControl::editChanged(int id,const QString& text)
{
    if (_updating) return;
    bool ok = 0;
...
void QdSeNumberControl::updateControl()
{
    _updating = 1;
...
void QdSeNumberControl::setValue(float value)
{
...
QdSeVectorControl::QdSeVectorControl(int id,QdSeVectorEditable* editable)
    :QdSeControl(id,editable,true),_numberEditable(editable)
{

    if(_numberEditable->isColor){
        _swatch=new QdCSwatch(editable->v);
    ...
    for(int i=0;i<3;i++){
        ...
        QdSeLineEdit* edit = new QdSeLineEdit(i, this);
        vbl->addWidget(edit);
        _edits[i]=edit;
        edit->setMinimumWidth(0);
        edit->setFixedHeight(16);

        QdSeChannelSlider* slider = new QdSeChannelSlider(i, this);
...
void QdSeVectorControl::swatchChanged(QColor gah)
{
...
QColor QdSeVectorControl::getColor()
{
...
void QdSeVectorControl::setColor(QColor color)
{
...
void QdSeVectorControl::sliderChanged(int id,float value)
{
    if (_updating) return;
...
void QdSeVectorControl::editChanged(int id,const QString& text)
{
    if (_updating) return;
    bool ok = 0;
    float val = text.toFloat(&ok);
...
void QdSeVectorControl::updateControl()
{
...
void QdSeVectorControl::setValue(int n,float value)
{
    if (n < 0 || n >= 3) return;
...
QdSeStringControl::QdSeStringControl(int id,QdSeStringEditable* editable)
    :QdSeControl(id,editable,false),_stringEditable(editable)
{
    // make line edit
    _edit=new QLineEdit();
...
void QdSeStringControl::fileBrowse()
{
    QdSeFileDialog dialog(this);
    dialog.setPreview();
...
void QdSeStringControl::directoryBrowse()
{
    QdSeFileDialog dialog(this);
    dialog.setPreview();
...
void QdSeStringControl::updateControl()
{
    _edit->setText(_stringEditable->v.c_str());
}

void QdSeStringControl::textChanged(const QString& newText)
{
    if(_updating) return;
...
QdSeCurveControl::QdSeCurveControl(int id,QdSeCurveEditable* editable)
    :QdSeControl(id,editable,false),_curveEditable(editable)
{
    _curve = new QdSeCurve(this, "Pos:", "Val:", "Interp:");
...
void QdSeCurveControl::curveChanged()
{
...
QdSeCCurveControl::QdSeCCurveControl(int id,QdSeCCurveEditable* editable)
    :QdSeControl(id,editable,true),_curveEditable(editable)
{
    _curve = new QdSeCCurve(this, "Pos:", "Val:", "Interp:");
    _curve->setFixedHeight(80);
...
void QdSeCCurveControl::curveChanged()
{
    if(_curve && _curveEditable){
        _curveEditable->cvs=_curve->_scene->_cvs;
        emit controlChanged(_id);
...
QdSeAnimCurveControl::QdSeAnimCurveControl(int id,QdSeAnimCurveEditable* editable)
:QdSeControl(id,editable,false),_editable(editable)
{

    _preview=new QdSeGraphPreview();
...
void QdSeAnimCurveControl::refreshClicked()
{
    if(callback){
#ifdef SEEXPR_USE_ANIMLIB
...
void QdSeAnimCurveControl::editGraphClicked(){
#ifdef SEEXPR_USE_ANIMLIB
...
void QdSeAnimCurveControl::setAnimCurveCallback(AnimCurveCallback newCallback)
{
    callback=newCallback;
}

QdSeAnimCurveControl::AnimCurveCallback QdSeAnimCurveControl::callback=0;
