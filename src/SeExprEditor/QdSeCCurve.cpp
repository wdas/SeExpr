...
void QdCBox::paintEvent(QPaintEvent* event)
{
    QPainter p(this);
    p.drawPixmap(0, 0, _curveScene->getPixmap());
}

void QdCSwatch::paintEvent(QPaintEvent* event)
{
    QPainter p(this);
...
QdCSwatch::QdCSwatch(SeVec3d value, QWidget* parent) : QFrame(parent), _value(value)
{
    _color = QColor(int(255 * _value[0] + 0.5), int(255 * _value[1] + 0.5), int(255 * _value[2] + 0.5));
}


void QdCSwatch::setValue(const SeVec3d &value)
{
...
SeVec3d QdCSwatch::getValue() const
{
    return _value;
}


void QdCSwatch::mousePressEvent(QMouseEvent* event)
{
#ifdef SEEXPR_USE_QDGUI
...
QdSeCCurve::QdSeCCurve(QWidget* parent, QString pLabel, QString vLabel, QString iLabel,
    bool expandable) :
    QWidget(parent),  _scene(0), _selPosEdit(0), _selValEdit(0), _interpComboBox(0)
{
    ...
    _selValEdit = new QdCSwatch(SeVec3d(.5));
...
void QdSeCCurve::cvSelectedSlot(const double pos, const SeVec3d val, const T_INTERP interp)
{
    QString posStr;
    if (pos >= 0.0) {
...
void QdSeCCurve::addPoint(const double x, const SeVec3d y, const T_INTERP interp, const bool select)
{
    _scene->addPoint(x, y, interp, select);
}

void QdSeCCurve::setSwatchColor(QColor color)
{
...
QColor QdSeCCurve::getSwatchColor()
{
    SeVec3d val=_selValEdit->getValue();
    return QColor::fromRgbF(val[0],val[1],val[2],1);
}

void QdSeCCurve::internalSwatchChanged(QColor color)
{
    emit swatchChanged(color);
}

void QdSeCCurve::openDetail()
{
    QDialog* dialog=new QDialog();
    dialog->setMinimumWidth(1024);
    dialog->setMinimumHeight(400);
    QdSeCCurve* curve=new QdSeCCurve(0,"","","",false);
