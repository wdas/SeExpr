...
struct GrayPixel {
    ...
    static constexpr size_t stride() { return 1; }
...
struct RGBPixel {
    ...
    static constexpr size_t stride() { return 3; }
    static void writeHeader(std::ostream& os, int width, int height) {
        os << "P6" << std::endl;
        os << width << " " << height << std::endl;
        os << +std::numeric_limits<T>::max() << std::endl;
...
class RowAccessor {
  public:
    ...
    PixelT& operator[](size_t col) { return _image->data()[_row * _image->width() + col]; }
...
class ConstRowAccessor {
  public:
    ...
    const PixelT& operator[](size_t col) const { return _image->data()[_row * _image->width() + col]; }
...
class Image {
  public:
    ...
    inline size_t numPixels() const { return _width * _height; }
    inline size_t bytes() const { return numPixels() * sizeof(PixelT); }
    inline int width() const { return _width; }
    inline int height() const { return _height; }

    PixelT* data() { return _data.get(); }
    const PixelT* data() const { return _data.get(); }

    RowAccessor<PixelT> operator[](size_t row) { return RowAccessor<PixelT>(this, row); }

    ConstRowAccessor<PixelT> operator[](size_t row) const { return RowAccessor<PixelT>(this, row); }
