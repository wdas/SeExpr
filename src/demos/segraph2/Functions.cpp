...
void Functions::setVar(const std::string& name, const double val)
{
...
bool Functions::isValid(int functionId) const
{
...
int Functions::rowCount(const QModelIndex&) const
{
    return functions.size();
}

int Functions::columnCount(const QModelIndex&) const
{
    return 2;
}
...
bool Functions::setData(const QModelIndex& index, const QVariant& value, int role)
{
    if (role == Qt::EditRole && index.column() == 1 && index.isValid()) {
        ...
        if (functions[index.row()])
            delete functions[index.row()];
