...
int Func1VOp(int* opData, double* fp, char** c, std::vector<int>& callStack) {
    fp[opData[2]] = ((ExprFuncStandard::Func1v*)(c[opData[0]]))(copyRawVec3<double>(&fp[opData[1]]));
    return 1;
}
int Func2VOp(int* opData, double* fp, char** c, std::vector<int>& callStack) {
    fp[opData[3]] =
        ((ExprFuncStandard::Func2v*)(c[opData[0]]))(copyRawVec3<double>(&fp[opData[1]]), copyRawVec3<double>(&fp[opData[2]]));
    return 1;
}
int Func1VVOp(int* opData, double* fp, char** c, std::vector<int>& callStack) {
    Vec3d v = ((ExprFuncStandard::Func1vv*)(c[opData[0]]))(copyRawVec3<double>(&fp[opData[1]]));
    double* out = &fp[opData[2]];
    for (int k = 0; k < 3; k++) out[k] = v[k];
    return 1;
}
int Func2VVOp(int* opData, double* fp, char** c, std::vector<int>& callStack) {
    Vec3d v = ((ExprFuncStandard::Func2vv*)(c[opData[0]]))(copyRawVec3<double>(&fp[opData[1]]), copyRawVec3<double>(&fp[opData[2]]));
...
int FuncNVOp(int* opData, double* fp, char** c, std::vector<int>& callStack) {
    ...
    for (int k = 0; k < n; k++) copyRawVec3(vals[k], &fp[opData[k + 2]]);
    double* out = &fp[opData[n + 2]];
