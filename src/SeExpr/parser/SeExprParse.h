...
*/
    ...
    NodePtr expr1(){
        ...
        if(token==Lexer::QUESTION){
            ...
            NodePtr res2=expr();
            curr=NodePtr(new typename Policy::TernaryOp(rangec.toPrevious(),'?',std::move(curr),std::move(res1),std::move(res2)));
        }
        return curr;
    }
    ...
    NodePtr expr6(){
        ...
        while(token==Lexer::PLUS || token==Lexer::MINUS){
            ...
            curr=NodePtr(new typename Policy::BinaryOp(rangec.toPrevious(),code,std::move(curr),std::move(newChild))) ;
        }
        return curr;
    }
    ...
    NodePtr expr11(){
        if(token==Lexer::PAREN_OPEN){
            ...
            NodePtr ret=NodePtr(new typename Policy::String(tokenPosition,tokenText.substr(1,tokenText.size()-2)));
            getToken();
            return ret;
