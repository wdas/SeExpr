...
#define SEEXPR_AST_SUBCLASS(name) \
    struct name : public Base { \
        template<typename...Args> name(const Range& range,Args&&...args):Base(range,ASTType:: name,args...) {} \
    };
#define SEEXPR_AST_SUBCLASS_OP(name) \
    struct name : public Base { \
        template<typename...Args> name(const Range& range,char op,Args&&...args):Base(range,ASTType:: name,args...), \
        _op(op) {} \
    ...
    struct Call : public Base {
        Call(const Range& range,const std::string& symbolIn)
            :Base(range,ASTType::Call),func(symbolIn)
        {}
