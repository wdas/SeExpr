...
struct GlobalFP: public GlobalVal
{
    ...
    void eval(double *result) {for(int i=0;i<type().dim();i++) result[i] = val[i];}
...
ExprVarRef* DExpression::resolveVar(const std::string& name) const {
    ...
    for(std::set<DExpression*>::iterator I = operandExprs.begin(),
            E = operandExprs.end(); I != E; ++I)
        if((*I)->name()==name){
    ...
    for(std::set<GlobalVal*>::iterator I = operandVars.begin(),
            E = operandVars.end(); I != E; ++I)
        if((*I)->varName==name) {
...
Expressions::~Expressions() {
    for(std::set<DExpression*>::iterator I = AllExprs.begin(),
            E = AllExprs.end(); I != E; ++I)
        delete *I;

    for(std::set<GlobalVal*>::iterator I = AllExternalVars.begin(),
            E = AllExternalVars.end(); I != E; ++I)
        delete *I;
}
...
void Expressions::setVariable(VariableHandle handle, double *values, unsigned dim) {
    ...
    std::set<DExpression*> ret = getAffectedExpr(thisvar);
    for(std::set<DExpression*>::iterator I = ret.begin(),
            E = ret.end(); I != E; ++I)
        (*I)->eval();
}
...
bool Expressions::isValid() const {
    bool ret = true;
    for(std::set<DExpression*>::const_iterator I = AllExprs.begin(),
            E = AllExprs.end(); I != E; ++I)
        ret &= (*I)->isValid();
    return ret;
}
...
ExprEvalHandle Expressions::getExprEvalHandle(ExprHandle eh) {
    ...
    it = std::set_intersection(all.begin(), all.end(),
                               exprToEval.begin(), exprToEval.end(),
                               ret1.begin());
    ret1.resize(it-ret1.begin());

    std::vector<DExpression*> ret2(ret1.size());
    it = std::set_difference(ret1.begin(), ret1.end(),
                             exprEvaled.begin(), exprEvaled.end(),
                             ret2.begin());
...
const std::vector<double> &Expressions::evalFP(ExprEvalHandle eeh) {
    ...
    for(std::vector<DExpression*>::iterator I = eeh.second.begin(),
            E = eeh.second.end(); I != E; ++I)
        (*I)->eval();
