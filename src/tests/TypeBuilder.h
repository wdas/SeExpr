...
class TypeBuilderExpr : public Expression {
  public:
    ...
    {
        if (to == "v")
            v = ExprNothingVarRef(resolveVar(from)->type());
        if (to == "x")
            x = ExprNothingVarRef(resolveVar(from)->type());
        if (to == "y")
            y = ExprNothingVarRef(resolveVar(from)->type());
        if (to == "z")
            z = ExprNothingVarRef(resolveVar(from)->type());
        // TODO: rem ove
