...
void TypeTesterExpr::doTest(const std::string & testStr,
                            ...
                            ExprType actualResult)
{
    ...
    if(actualResult.isValid()){
        EXPECT_TRUE(isValid());
        EXPECT_TRUE(returnType() == actualResult)
            << "         Expression: " << testStr
...
{
    return type.isFP() ? type : ExprType().Error().Varying(); return type;
};
...
ExprType
numericToScalar(const ExprType & first,
...
ExprType generalComparison(const ExprType & first,
                           const ExprType & second)
{
    if(ExprType::valuesCompatible(first,second)){
        ExprType t=ExprType().FP(1);t.setLifetime(first,second);
        return t;
    } else return ExprType().Error();
};

ExprType numericComparison(const ExprType & first,
                           const ExprType & second)
{
...
ExprType numericToNumeric(const ExprType & first,
                          const ExprType & second)
{
...
ExprType numericTo2Vector(const ExprType & first,
                          const ExprType & second)
{
    ExprType ret=ExprType().Error();
