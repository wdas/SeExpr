...
#define SPEC_IS_NUMBER(x) (dynamic_cast<ExprSpecScalarNode*>(x) != 0)
#define SPEC_IS_VECTOR(x) (dynamic_cast<ExprSpecVectorNode*>(x) != 0)
#define SPEC_IS_STR(x) (dynamic_cast<ExprSpecStringNode*>(x) != 0)
    ...
    if (!node) {
        ...
        if (ExprSpecListNode* args = dynamic_cast<ExprSpecListNode*>(n->args)) {
            if (args->nodes.size() == 12) {
                ...
                if (resolution && tileSize && lengthCutoff && amplitude && windAngle && windSpeed &&
                    directionalFactorExponent && directionalReflectionDamping && flowDirection && sharpen && time &&
                    filterWidth) {
                    deepWater->setParams(SeDeepWaterParams(resolution->v, tileSize->v, lengthCutoff->v, amplitude->v,
                                                           windAngle->v, windSpeed->v, directionalFactorExponent->v,
                                                           directionalReflectionDamping->v, flowDirection->v,
                                                           sharpen->v, time->v, filterWidth->v));
                } else {
                    valid = false;
                }

                if (valid)
                    editables->push_back(deepWater);
                else
                    delete deepWater;
            }
        }
...
static int ExprSpecYYID(int ExprSpeci)
#else
static int ExprSpecYYID(ExprSpeci) int ExprSpeci;
#endif
{
    return ExprSpeci;
}
...
#define ExprSpecYYSTACK_FREE(Ptr) \
    do { /* empty */              \
        ;                         \
...
#define ExprSpecYYSTACK_ALLOC_MAXIMUM ExprSpecYYSIZE_MAXIMUM
     ...
     !((defined ExprSpecYYMALLOC || defined malloc) && (defined ExprSpecYYFREE || defined free)))
#include <stdlib.h> /* INFRINGES ON USER NAME SPACE */
#ifndef EXIT_SUCCESS
#define EXIT_SUCCESS 0
#endif
...
void free(void*);               /* INFRINGES ON USER NAME SPACE */
     ...
     (!defined __cplusplus || (defined ExprSpecYYLTYPE_IS_TRIVIAL && ExprSpecYYLTYPE_IS_TRIVIAL && \
...
#define ExprSpecYYCOPY(Dst, Src, Count) __builtin_memcpy(Dst, Src, (Count) * sizeof(*(Src)))
#else
...
static const ExprSpectype_uint16 ExprSpecrline[] = {
    0,   313, 313, 314, 319, 320, 324, 325, 330, 331, 335, 336, 337, 338, 339, 340, 341, 345, 346, 347,
    348, 349, 350, 354, 359, 360, 361, 366, 367, 372, 373, 374, 375, 376, 377, 378, 379, 380, 381, 382,
    383, 392, 393, 394, 395, 396, 397, 398, 399, 400, 425, 426, 427, 428, 433, 434, 439, 448, 460, 461};
#endif
...
static const char* const ExprSpectname[] = {
    ...
    "'<'",    "'>'",        "GE",         "LE",     "'+'",     "'-'",   "'*'",        "'/'",
    "'%'",    "'!'",        "'~'",        "UNARY",  "'^'",     "'['",   "'='",        "';'",
    "'{'",    "'}'",        "','",        "']'",    "$accept", "expr",  "optassigns", "assigns",
    "assign", "ifthenelse", "optelse",    "e",      "optargs", "args",  "arg",        ExprSpecYY_NULL};
#endif
...
static const ExprSpectype_uint8 ExprSpecr1[] = {0,  44, 45, 45, 46, 46, 47, 47, 48, 48, 48, 48, 48, 48, 48,
...
static const ExprSpectype_uint8 ExprSpecr2[] = {0, 2, 2, 1, 0, 1, 1, 2, 1, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4,
...
static const ExprSpectype_int16 ExprSpecdefgoto[] = {-1, 11, 127, 128, 13, 14, 134, 68, 69, 70, 71};
...
static const ExprSpectype_int8 ExprSpecpgoto[] = {-65, -65, -64, 77, -11, -55, -65, 0, -38, -65, -27};
...
#define ExprSpecpact_value_is_default(Yystate) (!!((Yystate) == (-65)))

#define ExprSpectable_value_is_error(Yytable_value) ExprSpecYYID(0)
...
#define ExprSpecYYLLOC_DEFAULT(Current, Rhs, N)                                                    \
    do                                                                                             \
        if (ExprSpecYYID(N)) {                                                                     \
            ...
            (Current).first_line = (Current).last_line = ExprSpecYYRHSLOC(Rhs, 0).last_line;       \
            (Current).first_column = (Current).last_column = ExprSpecYYRHSLOC(Rhs, 0).last_column; \
        }                                                                                          \
    while (ExprSpecYYID(0))
#endif
...
#else
static unsigned ExprSpec_location_print_(ExprSpeco, ExprSpeclocp) FILE* ExprSpeco;
...
#define ExprSpecYY_LOCATION_PRINT(File, Loc) ExprSpec_location_print_(File, &(Loc))
...
#define ExprSpecYY_SYMBOL_PRINT(Title, Type, Value, Location)     \
    do {                                                          \
        if (ExprSpecdebug) {                                      \
            ...
            ExprSpec_symbol_print(stderr, Type, Value, Location); \
...
/*ARGSUSED*/
#if (defined __STDC__ || defined __C99__FUNC__ || defined __cplusplus || defined _MSC_VER)
static void ExprSpec_symbol_value_print(FILE* ExprSpecoutput,
                                        int ExprSpectype,
                                        ExprSpecYYSTYPE const* const ExprSpecvaluep,
                                        ExprSpecYYLTYPE const* const ExprSpeclocationp)
#else
static void ExprSpec_symbol_value_print(ExprSpecoutput,
                                        ExprSpectype,
                                        ExprSpecvaluep,
                                        ExprSpeclocationp) FILE* ExprSpecoutput;
int ExprSpectype;
ExprSpecYYSTYPE const* const ExprSpecvaluep;
ExprSpecYYLTYPE const* const ExprSpeclocationp;
#endif
...
#if (defined __STDC__ || defined __C99__FUNC__ || defined __cplusplus || defined _MSC_VER)
static void ExprSpec_symbol_print(FILE* ExprSpecoutput,
                                  int ExprSpectype,
                                  ExprSpecYYSTYPE const* const ExprSpecvaluep,
                                  ExprSpecYYLTYPE const* const ExprSpeclocationp)
#else
static void ExprSpec_symbol_print(ExprSpecoutput, ExprSpectype, ExprSpecvaluep, ExprSpeclocationp) FILE* ExprSpecoutput;
int ExprSpectype;
ExprSpecYYSTYPE const* const ExprSpecvaluep;
ExprSpecYYLTYPE const* const ExprSpeclocationp;
#endif
...
#if (defined __STDC__ || defined __C99__FUNC__ || defined __cplusplus || defined _MSC_VER)
static void ExprSpec_stack_print(ExprSpectype_int16* ExprSpecbottom, ExprSpectype_int16* ExprSpectop)
#else
static void ExprSpec_stack_print(ExprSpecbottom, ExprSpectop) ExprSpectype_int16* ExprSpecbottom;
...
#if (defined __STDC__ || defined __C99__FUNC__ || defined __cplusplus || defined _MSC_VER)
static void ExprSpec_reduce_print(ExprSpecYYSTYPE* ExprSpecvsp, ExprSpecYYLTYPE* ExprSpeclsp, int ExprSpecrule)
#else
static void ExprSpec_reduce_print(ExprSpecvsp, ExprSpeclsp, ExprSpecrule) ExprSpecYYSTYPE* ExprSpecvsp;
...
#if (defined __STDC__ || defined __C99__FUNC__ || defined __cplusplus || defined _MSC_VER)
static ExprSpecYYSIZE_T ExprSpecstrlen(const char* ExprSpecstr)
#else
static ExprSpecYYSIZE_T ExprSpecstrlen(ExprSpecstr) const char* ExprSpecstr;
#endif
{
...
/* Copy ExprSpecYYSRC to ExprSpecYYDEST, returning the address of the terminating '\0' in
   ExprSpecYYDEST.  */
#if (defined __STDC__ || defined __C99__FUNC__ || defined __cplusplus || defined _MSC_VER)
static char* ExprSpecstpcpy(char* ExprSpecdest, const char* ExprSpecsrc)
#else
static char* ExprSpecstpcpy(ExprSpecdest, ExprSpecsrc) char* ExprSpecdest;
...
/* Copy to ExprSpecYYRES the contents of ExprSpecYYSTR after stripping away unnecessary
   ...
   would have been.  */
static ExprSpecYYSIZE_T ExprSpectnamerr(char* ExprSpecres, const char* ExprSpecstr)
{
    if (*ExprSpecstr == '"') {
...
static int ExprSpecsyntax_error(ExprSpecYYSIZE_T* ExprSpecmsg_alloc,
                                char** ExprSpecmsg,
        ...
        if (!ExprSpecpact_value_is_default(ExprSpecn)) {
            ...
            for (ExprSpecx = ExprSpecxbegin; ExprSpecx < ExprSpecxend; ++ExprSpecx)
                if (ExprSpeccheck[ExprSpecx + ExprSpecn] == ExprSpecx && ExprSpecx != ExprSpecYYTERROR &&
                    ...
                    {
                        ExprSpecYYSIZE_T ExprSpecsize1 =
                            ExprSpecsize + ExprSpectnamerr(ExprSpecYY_NULL, ExprSpectname[ExprSpecx]);
...
#undef ExprSpecYYCASE_
    ...
    if (*ExprSpecmsg_alloc < ExprSpecsize) {
        *ExprSpecmsg_alloc = 2 * ExprSpecsize;
        if (!(ExprSpecsize <= *ExprSpecmsg_alloc && *ExprSpecmsg_alloc <= ExprSpecYYSTACK_ALLOC_MAXIMUM))
...
/*ARGSUSED*/
#if (defined __STDC__ || defined __C99__FUNC__ || defined __cplusplus || defined _MSC_VER)
static void ExprSpecdestruct(const char* ExprSpecmsg,
                             int ExprSpectype,
                             ExprSpecYYSTYPE* ExprSpecvaluep,
                             ExprSpecYYLTYPE* ExprSpeclocationp)
#else
static void ExprSpecdestruct(ExprSpecmsg, ExprSpectype, ExprSpecvaluep, ExprSpeclocationp) const char* ExprSpecmsg;
int ExprSpectype;
ExprSpecYYSTYPE* ExprSpecvaluep;
ExprSpecYYLTYPE* ExprSpeclocationp;
#endif
...
int ExprSpecparse(void* ExprSpecYYPARSE_PARAM)
#else
int ExprSpecparse(ExprSpecYYPARSE_PARAM) void* ExprSpecYYPARSE_PARAM;
#endif
#else /* ! ExprSpecYYPARSE_PARAM */
#if (defined __STDC__ || defined __C99__FUNC__ || defined __cplusplus || defined _MSC_VER)
int ExprSpecparse(void)
    ...
    *ExprSpecssp = ExprSpecstate;
    ...
    if (ExprSpecss + ExprSpecstacksize - 1 <= ExprSpecssp) {
...
#ifdef ExprSpecoverflow
        {
            ...
            ExprSpecoverflow(ExprSpecYY_("memory exhausted"), &ExprSpecss1, ExprSpecsize * sizeof(*ExprSpecssp),
                             &ExprSpecvs1, ExprSpecsize * sizeof(*ExprSpecvsp), &ExprSpecls1,
                             ExprSpecsize * sizeof(*ExprSpeclsp), &ExprSpecstacksize);
...
ExprSpecbackup:
    ...
    ExprSpecYY_IGNORE_MAYBE_UNINITIALIZED_BEGIN* ++ExprSpecvsp = ExprSpeclval;
    ExprSpecYY_IGNORE_MAYBE_UNINITIALIZED_END* ++ExprSpeclsp = ExprSpeclloc;
...
#line 367 "/disney/users/jberlin/projects/seexpr2/src/ui/ExprSpecParser.y"
    {
        if (SPEC_IS_NUMBER((ExprSpecvsp[(2) - (7)].n)) && SPEC_IS_NUMBER((ExprSpecvsp[(4) - (7)].n)) &&
            SPEC_IS_NUMBER((ExprSpecvsp[(6) - (7)].n))) {
            (ExprSpecval.n) = remember(new ExprSpecVectorNode((ExprSpecloc).first_column, (ExprSpecloc).last_column,
                                                              (ExprSpecvsp[(2) - (7)].n), (ExprSpecvsp[(4) - (7)].n),
                                                              (ExprSpecvsp[(6) - (7)].n)));
        } else
            (ExprSpecval.n) = 0;
    } break;
...
#line 428 "/disney/users/jberlin/projects/seexpr2/src/ui/ExprSpecParser.y"
    {
        (ExprSpecval.n) = remember(
            new ExprSpecScalarNode((ExprSpecloc).first_column, (ExprSpecloc).last_column, (ExprSpecvsp[(1) - (1)].d)));
    } break;
...
#line 448 "/disney/users/jberlin/projects/seexpr2/src/ui/ExprSpecParser.y"
    {
        ...
        if ((ExprSpecvsp[(1) - (3)].n) && (ExprSpecvsp[(3) - (3)].n) &&
            ((SPEC_IS_NUMBER((ExprSpecvsp[(3) - (3)].n)) || SPEC_IS_VECTOR((ExprSpecvsp[(3) - (3)].n)) ||
              SPEC_IS_STR((ExprSpecvsp[(3) - (3)].n))))) {
...
#line 461 "/disney/users/jberlin/projects/seexpr2/src/ui/ExprSpecParser.y"
    {
        ExprSpecStringNode* str =
            new ExprSpecStringNode((ExprSpecloc).first_column, (ExprSpecloc).last_column, (ExprSpecvsp[(1) - (1)].s));
        // specRegisterEditable("<UNKNOWN>",str);
...
#else
#define ExprSpecYYSYNTAX_ERROR ExprSpecsyntax_error(&ExprSpecmsg_alloc, &ExprSpecmsg, ExprSpecssp, ExprSpectoken)
        {
...
#undef ExprSpecYYSYNTAX_ERROR
    ...
    if (ExprSpecerrstatus == 3) {
        ...
        if (ExprSpecchar <= ExprSpecYYEOF) {
            ...
            ExprSpecdestruct("Error: discarding", ExprSpectoken, &ExprSpeclval, &ExprSpeclloc);
            ExprSpecchar = ExprSpecYYEMPTY;
        }
    }
...
ExprSpecerrlab1:
    ...
    for (;;) {
        ...
        ExprSpecdestruct("Error: popping", ExprSpecstos[ExprSpecstate], ExprSpecvsp, ExprSpeclsp);
...
ExprSpecreturn:
    if (ExprSpecchar != ExprSpecYYEMPTY) {
        /* Make sure we have latest lookahead translation.  See comments at
           user semantic actions for why this is necessary.  */
        ExprSpectoken = ExprSpecYYTRANSLATE(ExprSpecchar);
        ExprSpecdestruct("Cleanup: discarding lookahead", ExprSpectoken, &ExprSpeclval, &ExprSpeclloc);
    }
    /* Do not reclaim the symbols of the rule which action triggered
    ...
    while (ExprSpecssp != ExprSpecss) {
        ExprSpecdestruct("Cleanup: popping", ExprSpecstos[*ExprSpecssp], ExprSpecvsp, ExprSpeclsp);
        ExprSpecYYPOPSTACK(1);
    }
                   ...
                   const char* str)
{
    ...
    for (size_t i = 0; i < specNodes.size(); i++)
        delete specNodes[i];
    specNodes.clear();
