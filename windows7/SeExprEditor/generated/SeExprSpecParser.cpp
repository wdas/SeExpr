...
#define SPEC_IS_NUMBER(x) (dynamic_cast<SeExprSpecScalarNode*>(x) != 0)
#define SPEC_IS_VECTOR(x) (dynamic_cast<SeExprSpecVectorNode*>(x) != 0)
#define SPEC_IS_STR(x) (dynamic_cast<SeExprSpecStringNode*>(x) != 0)
...
#elif !defined SeExprSpecYYSIZE_T && \
...
static int SeExprSpecYYID(int SeExprSpeci)
#else
static int SeExprSpecYYID(SeExprSpeci) int SeExprSpeci;
#endif
{
    return SeExprSpeci;
}
...
#define SeExprSpecYYSTACK_FREE(Ptr) \
    do { /* empty */                \
        ;                           \
...
#define SeExprSpecYYSTACK_ALLOC_MAXIMUM SeExprSpecYYSIZE_MAXIMUM
     ...
     !((defined SeExprSpecYYMALLOC || defined malloc) && (defined SeExprSpecYYFREE || defined free)))
#include <stdlib.h> /* INFRINGES ON USER NAME SPACE */
#ifndef _STDLIB_H
#define _STDLIB_H 1
#endif
...
void free(void*);                 /* INFRINGES ON USER NAME SPACE */
     ...
     (!defined __cplusplus || (defined SeExprSpecYYLTYPE_IS_TRIVIAL && SeExprSpecYYLTYPE_IS_TRIVIAL && \
...
#define SeExprSpecYYCOPY(To, From, Count) __builtin_memcpy(To, From, (Count) * sizeof(*(From)))
#else
...
static const SeExprSpectype_uint16 SeExprSpecrline[] = {
    0,   266, 266, 267, 272, 273, 277, 278, 283, 284, 288, 289, 290, 291, 292, 293, 294, 298, 299, 300,
    301, 302, 303, 307, 312, 313, 314, 319, 320, 325, 326, 327, 328, 329, 330, 331, 332, 333, 334, 335,
    336, 345, 346, 347, 348, 349, 350, 351, 352, 353, 374, 375, 376, 377, 382, 383, 388, 397, 409, 410};
#endif
...
static const SeExprSpectype_uint8 SeExprSpecr1[] = {0,  44, 45, 45, 46, 46, 47, 47, 48, 48, 48, 48, 48, 48, 48,
...
static const SeExprSpectype_uint8 SeExprSpecr2[] = {0, 2, 2, 1, 0, 1, 1, 2, 1, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4,
...
static const SeExprSpectype_int16 SeExprSpecdefgoto[] = {-1, 11, 127, 128, 13, 14, 134, 68, 69, 70, 71};
...
static const SeExprSpectype_int8 SeExprSpecpgoto[] = {-65, -65, -64, 77, -11, -55, -65, 0, -38, -65, -27};
...
#define SeExprSpecYYLLOC_DEFAULT(Current, Rhs, N)                                                    \
    do                                                                                               \
        if (SeExprSpecYYID(N)) {                                                                     \
            ...
            (Current).first_line = (Current).last_line = SeExprSpecYYRHSLOC(Rhs, 0).last_line;       \
            (Current).first_column = (Current).last_column = SeExprSpecYYRHSLOC(Rhs, 0).last_column; \
        }                                                                                            \
    while (SeExprSpecYYID(0))
#endif
...
#define SeExprSpecYY_SYMBOL_PRINT(Title, Type, Value, Location)     \
    do {                                                            \
        if (SeExprSpecdebug) {                                      \
            ...
            SeExprSpec_symbol_print(stderr, Type, Value, Location); \
...
/*ARGSUSED*/
#if (defined __STDC__ || defined __C99__FUNC__ || defined __cplusplus || defined _MSC_VER)
static void SeExprSpec_symbol_value_print(FILE* SeExprSpecoutput,
                                          int SeExprSpectype,
                                          SeExprSpecYYSTYPE const* const SeExprSpecvaluep,
                                          SeExprSpecYYLTYPE const* const SeExprSpeclocationp)
#else
static void SeExprSpec_symbol_value_print(SeExprSpecoutput,
                                          SeExprSpectype,
                                          SeExprSpecvaluep,
                                          SeExprSpeclocationp) FILE* SeExprSpecoutput;
int SeExprSpectype;
SeExprSpecYYSTYPE const* const SeExprSpecvaluep;
SeExprSpecYYLTYPE const* const SeExprSpeclocationp;
#endif
...
#if (defined __STDC__ || defined __C99__FUNC__ || defined __cplusplus || defined _MSC_VER)
static void SeExprSpec_symbol_print(FILE* SeExprSpecoutput,
                                    int SeExprSpectype,
                                    SeExprSpecYYSTYPE const* const SeExprSpecvaluep,
                                    SeExprSpecYYLTYPE const* const SeExprSpeclocationp)
#else
static void SeExprSpec_symbol_print(SeExprSpecoutput,
                                    SeExprSpectype,
                                    SeExprSpecvaluep,
                                    SeExprSpeclocationp) FILE* SeExprSpecoutput;
int SeExprSpectype;
...
#if (defined __STDC__ || defined __C99__FUNC__ || defined __cplusplus || defined _MSC_VER)
static void SeExprSpec_stack_print(SeExprSpectype_int16* SeExprSpecbottom, SeExprSpectype_int16* SeExprSpectop)
#else
static void SeExprSpec_stack_print(SeExprSpecbottom, SeExprSpectop) SeExprSpectype_int16* SeExprSpecbottom;
...
#if (defined __STDC__ || defined __C99__FUNC__ || defined __cplusplus || defined _MSC_VER)
static void SeExprSpec_reduce_print(SeExprSpecYYSTYPE* SeExprSpecvsp,
                                    SeExprSpecYYLTYPE* SeExprSpeclsp,
                                    int SeExprSpecrule)
#else
static void SeExprSpec_reduce_print(SeExprSpecvsp, SeExprSpeclsp, SeExprSpecrule) SeExprSpecYYSTYPE* SeExprSpecvsp;
SeExprSpecYYLTYPE* SeExprSpeclsp;
int SeExprSpecrule;
#endif
{
...
#if (defined __STDC__ || defined __C99__FUNC__ || defined __cplusplus || defined _MSC_VER)
static SeExprSpecYYSIZE_T SeExprSpecstrlen(const char* SeExprSpecstr)
#else
static SeExprSpecYYSIZE_T SeExprSpecstrlen(SeExprSpecstr) const char* SeExprSpecstr;
#endif
{
...
/* Copy SeExprSpecYYSRC to SeExprSpecYYDEST, returning the address of the terminating '\0' in
   SeExprSpecYYDEST.  */
#if (defined __STDC__ || defined __C99__FUNC__ || defined __cplusplus || defined _MSC_VER)
static char* SeExprSpecstpcpy(char* SeExprSpecdest, const char* SeExprSpecsrc)
#else
static char* SeExprSpecstpcpy(SeExprSpecdest, SeExprSpecsrc) char* SeExprSpecdest;
...
/* Copy to SeExprSpecYYRES the contents of SeExprSpecYYSTR after stripping away unnecessary
   ...
   would have been.  */
static SeExprSpecYYSIZE_T SeExprSpectnamerr(char* SeExprSpecres, const char* SeExprSpecstr)
{
    if (*SeExprSpecstr == '"') {
...
/* Copy into SeExprSpecYYRESULT an error message about the unexpected token
   ...
   size calculation.  */
static SeExprSpecYYSIZE_T SeExprSpecsyntax_error(char* SeExprSpecresult, int SeExprSpecstate, int SeExprSpecchar)
{
    ...
    if (!(SeExprSpecYYPACT_NINF < SeExprSpecn && SeExprSpecn <= SeExprSpecYYLAST))
	 ...
	 constructed on the fly.  */
      ...
      SeExprSpecYY_("syntax error, unexpected %s, expecting %s or %s or %s or %s");
        ...
        char SeExprSpecformat[sizeof SeExprSpecunexpected + sizeof SeExprSpecexpecting - 1 +
                              ((SeExprSpecYYERROR_VERBOSE_ARGS_MAXIMUM - 2) * (sizeof SeExprSpecor - 1))];
...
/*ARGSUSED*/
#if (defined __STDC__ || defined __C99__FUNC__ || defined __cplusplus || defined _MSC_VER)
static void SeExprSpecdestruct(const char* SeExprSpecmsg,
                               int SeExprSpectype,
                               SeExprSpecYYSTYPE* SeExprSpecvaluep,
                               SeExprSpecYYLTYPE* SeExprSpeclocationp)
#else
static void SeExprSpecdestruct(SeExprSpecmsg, SeExprSpectype, SeExprSpecvaluep, SeExprSpeclocationp) const
    char* SeExprSpecmsg;
int SeExprSpectype;
SeExprSpecYYSTYPE* SeExprSpecvaluep;
SeExprSpecYYLTYPE* SeExprSpeclocationp;
...
int SeExprSpecparse(void* SeExprSpecYYPARSE_PARAM)
#else
int SeExprSpecparse(SeExprSpecYYPARSE_PARAM) void* SeExprSpecYYPARSE_PARAM;
#endif
#else /* ! SeExprSpecYYPARSE_PARAM */
#if (defined __STDC__ || defined __C99__FUNC__ || defined __cplusplus || defined _MSC_VER)
int SeExprSpecparse(void)
    ...
    *SeExprSpecssp = SeExprSpecstate;
    ...
    if (SeExprSpecss + SeExprSpecstacksize - 1 <= SeExprSpecssp) {
...
#ifdef SeExprSpecoverflow
        {
            ...
            SeExprSpecoverflow(SeExprSpecYY_("memory exhausted"), &SeExprSpecss1,
                               SeExprSpecsize * sizeof(*SeExprSpecssp), &SeExprSpecvs1,
                               SeExprSpecsize * sizeof(*SeExprSpecvsp), &SeExprSpecls1,
                               SeExprSpecsize * sizeof(*SeExprSpeclsp), &SeExprSpecstacksize);
...
#line 320 "/home/fahome/jberlin/projects/SeExpr.linux/src/SeExprEditor/SeExprSpecParser.y"
    {
        if (SPEC_IS_NUMBER((SeExprSpecvsp[(2) - (7)].n)) && SPEC_IS_NUMBER((SeExprSpecvsp[(4) - (7)].n)) &&
            SPEC_IS_NUMBER((SeExprSpecvsp[(6) - (7)].n))) {
            (SeExprSpecval.n) = remember(new SeExprSpecVectorNode(
                (SeExprSpecloc).first_column, (SeExprSpecloc).last_column, (SeExprSpecvsp[(2) - (7)].n),
                (SeExprSpecvsp[(4) - (7)].n), (SeExprSpecvsp[(6) - (7)].n)));
        } else
            (SeExprSpecval.n) = 0;
    } break;
...
#line 377 "/home/fahome/jberlin/projects/SeExpr.linux/src/SeExprEditor/SeExprSpecParser.y"
    {
        (SeExprSpecval.n) = remember(new SeExprSpecScalarNode((SeExprSpecloc).first_column, (SeExprSpecloc).last_column,
                                                              (SeExprSpecvsp[(1) - (1)].d)));
    } break;
...
#line 397 "/home/fahome/jberlin/projects/SeExpr.linux/src/SeExprEditor/SeExprSpecParser.y"
    {
        ...
        if ((SeExprSpecvsp[(1) - (3)].n) && (SeExprSpecvsp[(3) - (3)].n) &&
            ((SPEC_IS_NUMBER((SeExprSpecvsp[(3) - (3)].n)) || SPEC_IS_VECTOR((SeExprSpecvsp[(3) - (3)].n)) ||
              SPEC_IS_STR((SeExprSpecvsp[(3) - (3)].n))))) {
...
#line 410 "/home/fahome/jberlin/projects/SeExpr.linux/src/SeExprEditor/SeExprSpecParser.y"
    {
        SeExprSpecStringNode* str = new SeExprSpecStringNode((SeExprSpecloc).first_column, (SeExprSpecloc).last_column,
                                                             (SeExprSpecvsp[(1) - (1)].s));
        // registerEditable("<UNKNOWN>",str);
...
#line 2312 "y.tab.c"
    ...
    *++SeExprSpeclsp = SeExprSpecloc;
    ...
    if (0 <= SeExprSpecstate && SeExprSpecstate <= SeExprSpecYYLAST &&
        SeExprSpeccheck[SeExprSpecstate] == *SeExprSpecssp)
...
SeExprSpecerrlab:
    ...
    if (SeExprSpecerrstatus == 3) {
        ...
        if (SeExprSpecchar <= SeExprSpecYYEOF) {
            ...
            SeExprSpecdestruct("Error: discarding", SeExprSpectoken, &SeExprSpeclval, &SeExprSpeclloc);
            SeExprSpecchar = SeExprSpecYYEMPTY;
        }
    }
...
SeExprSpecerrlab1:
    ...
    for (;;) {
        ...
        if (SeExprSpecn != SeExprSpecYYPACT_NINF) {
            ...
            if (0 <= SeExprSpecn && SeExprSpecn <= SeExprSpecYYLAST &&
                SeExprSpeccheck[SeExprSpecn] == SeExprSpecYYTERROR) {
                SeExprSpecn = SeExprSpectable[SeExprSpecn];
                if (0 < SeExprSpecn)
                    break;
            }
        ...
        SeExprSpecdestruct("Error: popping", SeExprSpecstos[SeExprSpecstate], SeExprSpecvsp, SeExprSpeclsp);
        SeExprSpecYYPOPSTACK(1);
        SeExprSpecstate = *SeExprSpecssp;
        SeExprSpecYY_STACK_PRINT(SeExprSpecss, SeExprSpecssp);
    }
...
SeExprSpecreturn:
    if (SeExprSpecchar != SeExprSpecYYEMPTY)
        SeExprSpecdestruct("Cleanup: discarding lookahead", SeExprSpectoken, &SeExprSpeclval, &SeExprSpeclloc);
    /* Do not reclaim the symbols of the rule which action triggered
       this SeExprSpecYYABORT or SeExprSpecYYACCEPT.  */
    SeExprSpecYYPOPSTACK(SeExprSpeclen);
    SeExprSpecYY_STACK_PRINT(SeExprSpecss, SeExprSpecssp);
    while (SeExprSpecssp != SeExprSpecss) {
        SeExprSpecdestruct("Cleanup: popping", SeExprSpecstos[*SeExprSpecssp], SeExprSpecvsp, SeExprSpeclsp);
        SeExprSpecYYPOPSTACK(1);
    }
